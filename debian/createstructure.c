...
void createstructure()
{
	...
	sprintf (execution_string, "python3 -m pip freeze | grep createstructure | sed 's/createstructure==//'; python3 -c \"exec(\\\"from createstructure import createstructure;createstructure()\\\")\" -t=%s -o=%s -i=%s %s", TOKEN, ORGANIZATION_NAME, IGNORE, (verbose ? "-v" : ""));
#endif // UBUNTU
#ifdef WINDOWS
    ...
    if (verbose)
        printf("%s\n", execution_string);
	system(execution_string);
	system("sudo apt install python3-pip; python3 -m pip freeze");
